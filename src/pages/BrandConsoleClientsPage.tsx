...
import { brandConsoleApi } from '../lib/api'
...
// Brand-scoped clone of AppsPage — same table/modal, but every OAuth2
// client here belongs to the brand as a whole (see BrandConsoleController),
// not to one org within it. Typically a brand registers one client for its
// own frontend/backend deployment (see SsoController.java on the SMAT side).
export default function BrandConsoleClientsPage() {
  ...
  async function load() {
    setLoading(true)
    try {
      const data = await brandConsoleApi.listClients(brandSlug!)
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    setFormError('')
    try {
      const res: any = await brandConsoleApi.createClient(brandSlug!, {
      ...
      setFormError(err?.response?.data?.message || 'Failed to register client.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  async function handleDelete(clientId: string) {
    await brandConsoleApi.deleteClient(brandSlug!, clientId)
  ...
  return (
    <div style={{ padding: '1.75rem' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <div>
          <h1 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-1)', marginBottom: '0.25rem' }}>Clients</h1>
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-2)' }}>
            OAuth2 clients registered to your brand
          </p>
        </div>
      ...
      <AnimatePresence>
        {newSecret && (
          <motion.div
            variants={MODAL_BG} initial="hidden" animate="visible" exit="exit"
            style={{
              display: 'flex', alignItems: 'flex-start', gap: '0.875rem', padding: '1rem 1.125rem',
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <code style={{ fontSize: '0.75rem', color: 'var(--text-2)', fontFamily: "'JetBrains Mono', monospace", wordBreak: 'break-all', display: 'block', marginBottom: '0.25rem' }}>
                {newSecret.secret}
              </code>
              <p style={{ fontSize: '0.72rem', color: 'var(--text-3)' }}>Client ID: {newSecret.clientId}</p>
            </div>
            ...
            <button className="copy-btn" onClick={() => setNewSecret(null)} title="Dismiss"><X size={14} /></button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="card" style={{ overflow: 'hidden', background: 'var(--surface)' }}>
        {loading ? (
          ...
          <div className="empty-state">
            <AppWindow size={32} />
            <p style={{ fontWeight: 500, color: 'var(--text-2)', fontSize: '0.9rem' }}>No clients registered yet</p>
            <p style={{ fontSize: '0.8125rem' }}>Register your brand's OAuth2 client to get started.</p>
          ...
          <div className="table-wrapper">
            <table>
              ...
              <tbody>
                {clients.map((c, i) => (
                  <motion.tr key={c.id || c.clientId} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                        ...
                        <span style={{ color: 'var(--text-1)', fontWeight: 500, fontSize: '0.875rem' }}>{c.clientName}</span>
                      </div>
                    </td>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <code style={{ fontSize: '0.72rem', color: 'var(--text-2)', fontFamily: "'JetBrains Mono', monospace", background: 'var(--surface-2)', border: '1px solid var(--border)', padding: '0.2rem 0.5rem', borderRadius: '0.3rem', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>
                          {c.clientId.slice(0, 14)}…
                        </code>
                        <button className="copy-btn" onClick={() => copy(c.clientId, c.clientId)} title="Copy Client ID">
                          {copied === c.clientId ? <Check size={12} style={{ color: 'var(--success)' }} /> : <Copy size={12} />}
                        </button>
                      </div>
                    </td>
                    <td>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                        {c.grantTypes.map(g => <span key={g} className="badge badge-indigo">{g.replace(/_/g, ' ')}</span>)}
                      </div>
                    </td>
                    <td>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                        {c.scopes.map(s => <span key={s} className="badge badge-gray">{s}</span>)}
                      </div>
                    </td>
                    <td>
                      <button className="btn-danger" onClick={() => handleDelete(c.clientId)} title="Delete client" style={{ padding: '0.375rem' }}>
                        <Trash2 size={14} />
                      </button>
                    </td>
                  </motion.tr>
          ...
          <motion.div className="modal-overlay" variants={MODAL_BG} initial="hidden" animate="visible" exit="exit" onClick={() => setShowModal(false)}>
            <motion.div variants={MODAL_CARD} initial="hidden" animate="visible" exit="exit" onClick={e => e.stopPropagation()} className="card" style={{ width: '100%', maxWidth: 468, padding: '1.5rem', background: 'var(--surface)', border: '1px solid var(--border-2)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
                <h3 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--text-1)' }}>Register Client</h3>
                <button className="btn-ghost" onClick={() => setShowModal(false)} style={{ padding: '0.25rem' }}><X size={16} /></button>
              </div>

              <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  <p className="section-label">Client Name</p>
                  <input className="input" placeholder="My App" value={form.clientName} onChange={e => setForm(f => ({ ...f, clientName: e.target.value }))} required autoFocus />
                </div>
                <div>
                  ...
                  <textarea className="input" rows={3} placeholder="https://myapp.com/callback" value={form.redirectUris} onChange={e => setForm(f => ({ ...f, redirectUris: e.target.value }))} style={{ resize: 'vertical', minHeight: '4.5rem' }} />
                </div>
                <div>
                  ...
                  <input className="input" placeholder="https://myapp.com/api/v1/permissions/catalog" value={form.permissionsUri} onChange={e => setForm(f => ({ ...f, permissionsUri: e.target.value }))} />
                </div>
                <div>
                  <p className="section-label">Scopes</p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                    {SCOPES.map(s => <ToggleChip key={s} label={s} active={form.scopes.includes(s)} onClick={() => toggleScope(s)} />)}
                  </div>
                </div>
                <div>
                  <p className="section-label">Grant Types</p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                    {GRANTS.map(g => <ToggleChip key={g} label={g.replace(/_/g, ' ')} active={form.grantTypes.includes(g)} onClick={() => toggleGrant(g)} />)}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', paddingTop: '0.5rem' }}>
                  ...
                  <button type="submit" className="btn-primary" disabled={submitting || !form.clientName} style={{ flex: 1, justifyContent: 'center' }}>
                    {submitting ? <Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} /> : 'Register'}
                  </button>
                </div>
              </form>
            </motion.div>
